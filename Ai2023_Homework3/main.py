...
if __name__ == '__main__':
    ...
    data1 = pd.read_csv('sex.csv')
    sexs = data1['sex'].values
    # 设定初始参数
    n_components = 2
    ...
    sexs[sexs == 'M'] = np.argmax(np.bincount(pred))
    sexs[sexs == 'F'] = np.argmin(np.bincount(pred))
    result = pred - sexs

    print('Accuracy:',result[ result == 0 ].size/n_samples)
